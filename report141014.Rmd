...
## NAを0に置換
bbb1 <- as.data.frame(apply(aaa, 2 , function(x){replace(x, which(is.na(x)), 0)}))
bbb1[2:13] <- as.data.frame(apply(bbb1[2:13], 2 , function(x){as.numeric(x)}))

## 月々の来店回数の平均, 標準偏差, max, median, min, diffの列を追加
bbb1$mean <- apply(bbb1[2:13], 1, mean)
bbb1$sd <- apply(bbb1[2:13], 1, sd)
bbb1$max <- apply(bbb1[2:13], 1, max)
bbb1$med <- apply(bbb1[2:13], 1, median)
bbb1$min <- apply(bbb1[2:13], 1, min)
bbb1$diff <- bbb1$max-bbb1$min

## meanとsdの散布図
ggplot(bbb1, aes(mean, sd)) + geom_point(alpha=0.5, shape=21)

## medとsdの散布図
ggplot(bbb1, aes(med, sd)) + geom_point(alpha=0.5, shape=21)
```
...
## NAを0に置換
bbb2 <- as.data.frame(apply(aaa, 2 , function(x){replace(x, which(is.na(x)), 0)}))
bbb2[2:13] <- as.data.frame(apply(bbb2[2:13], 2 , function(x){as.numeric(x)}))

## 月々の購入金額の平均, 標準偏差, max, median, min, diffの列を追加
bbb2$mean <- apply(bbb2[2:13], 1, mean)
bbb2$sd <- apply(bbb2[2:13], 1, sd)
bbb2$max <- apply(bbb2[2:13], 1, max)
bbb2$med <- apply(bbb2[2:13], 1, median)
bbb2$min <- apply(bbb2[2:13], 1, min)
bbb2$diff <- bbb2$max-bbb2$min

## meanとsdの散布図
ggplot(bbb2, aes(mean, sd)) + geom_point(alpha=0.5, shape=21)

## medとsdの散布図
ggplot(bbb2, aes(med, sd)) + geom_point(alpha=0.5, shape=21)

## デシル分析
bbb2 %.%
  arrange(desc(med)) %.%
  mutate(dl=med/sum(bbb2$med)*100) -> bbb3
# 上位20%まで1%刻み
bbb3$class <- 100
for(i in 0:19){
  j <- round(nrow(bbb3)/100)
  bbb3[(i*j):((i+1)*j),]$class <- (i+1)*1
}
# 20%から5%刻み
for(i in 4:19){
  j <- round(nrow(bbb3)/20)
  bbb3[(i*j):((i+1)*j),]$class <- (i+1)*5
}
bbb3$class <- factor(bbb3$class, levels=append(c(1:19), seq(20,100, by=5)))
bbb3 %.%
  select(dl, class) %.%
  group_by(class) %.%
  summarise(dl2=sum(dl)) -> bbb3
bbb3 %.%
  mutate(cum=cumsum(aaa$dl2)) %.%
  ggplot(aes(x=class, y=cum, fill)) + 
    geom_histogram(stat="identity") +
    labs(x="パーセンテージ", y="上位n％の購入金額")
```
...
## NAを0に置換
bbb3 <- as.data.frame(apply(aaa, 2 , function(x){replace(x, which(is.na(x)), 0)}))
bbb3[2:13] <- as.data.frame(apply(bbb3[2:13], 2 , function(x){as.numeric(x)}))

## 月々の購入金額の平均, 標準偏差, max, median, min, diffの列を追加
bbb3$mean <- apply(bbb3[2:13], 1, mean)
bbb3$sd <- apply(bbb3[2:13], 1, sd)
bbb3$max <- apply(bbb3[2:13], 1, max)
bbb3$med <- apply(bbb3[2:13], 1, median)
bbb3$min <- apply(bbb3[2:13], 1, min)
bbb3$diff <- bbb3$max-bbb3$min

## meanとsdの散布図
ggplot(bbb3, aes(mean, sd)) + geom_point(alpha=0.5, shape=21)

## medとsdの散布図
ggplot(bbb3, aes(med, sd)) + geom_point(alpha=0.5, shape=21)
```
...
## NAを0に置換
bbb4 <- as.data.frame(apply(aaa, 2 , function(x){replace(x, which(is.na(x)), 0)}))
bbb4[2:13] <- as.data.frame(apply(bbb4[2:13], 2 , function(x){as.numeric(x)}))

## 月々の購入数の平均, 標準偏差, max, median, min, diffの列を追加
bbb4$mean <- apply(bbb4[2:13], 1, mean)
bbb4$sd <- apply(bbb4[2:13], 1, sd)
bbb4$max <- apply(bbb4[2:13], 1, max)
bbb4$med <- apply(bbb4[2:13], 1, median)
bbb4$min <- apply(bbb4[2:13], 1, min)
bbb4$diff <- bbb4$max-bbb4$min

## meanとsdの散布図
ggplot(bbb4, aes(mean, sd)) + geom_point(alpha=0.5, shape=21)

## medとsdの散布図
ggplot(bbb4, aes(med, sd)) + geom_point(alpha=0.5, shape=21)
```

### 4-4-5. 優良顧客の推定
```{r, warning=FALSE, fig.width=20, fig.height=16}
# メモ：閾値を何かにして決めるか。または主成分分析系で決める
data <- data.frame(顧客CD=bbb1$顧客CD,
                   med1=bbb1$med,
                   med2=bbb2$med,
                   med3=bbb3$med,
                   med4=bbb4$med)
summary(data)
#data$med1 <- data$med1 - mean(data$med1)
#data$med2 <- data$med2 - mean(data$med2)
#data$med3 <- data$med3 - mean(data$med3)
#data$med4 <- data$med4 - mean(data$med4)
...
## メモ：たぶん，「優良顧客」と「安定顧客」と「非優良顧客」と「離反顧客」くらいに分けたらいいのかな
